    ...
    public static void init() {
        ...
        if (GregTechMod.classic) {
            ...
            ModHandler.addShapedRecipe(
                "glassFibreCable",
                ItemHandlerHelper.copyStackWithSize(glassFibreCable, 4),
                getCableRecipeIngredients("dustRedstone", null)
            );
            ModHandler.addShapedRecipe(
                "glassFibreCableSilver",
                ItemHandlerHelper.copyStackWithSize(glassFibreCable, 6),
                getCableRecipeIngredients("ingotSilver", "dustSilver")
            );
        }
        ModHandler.addShapedRecipe(
            "glassFibreCableElectrum",
            ItemHandlerHelper.copyStackWithSize(glassFibreCable, 8),
        ...
        if (GregTechConfig.DISABLED_RECIPES.enderChest)
            ModHandler.removeCraftingRecipe(new ItemStack(Blocks.ENDER_CHEST));
        ...
        ModHandler.addShapedRecipe(
            ...
            ItemHandlerHelper.copyStackWithSize(IC2Items.getItem("cable", "type:gold,insulation:2"), 4),
        ...
        ModHandler.removeCraftingRecipeFromInputs(planks, ItemStack.EMPTY, ItemStack.EMPTY, planks)
            .ifPresent(output -> {
                ...
                ModHandler.addShapedRecipe(
                    "sticksFromPlanks",
                    ItemHandlerHelper.copyStackWithSize(output, GregTechConfig.GENERAL.woodNeedsSawForCrafting ? output.getCount() / 2 : output.getCount()),
                    "P", "P", 'P', "plankWood"
                );
            });
